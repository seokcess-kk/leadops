...
### ❗ 실측된 것 (2026-08-06 측정 · 08-05 재크롤 · 실키 · shadow ORS · 라벨 90건 · 시드 42)

08-04 첫 측정의 M2 재현율 39.3% 를 FN 17건 분해로 원인을 좁혀 4건을 고친 뒤
(종별 접미 대조 #13 · 껍데기 리다이렉트 추적 · 죽은 후보 재발견 · Crawl-delay 단일 요청 #14)
재크롤·재측정한 결과다.

| 지표 | 실측 | 기준 | 비고 |
|---|---:|---:|---|
| **M1 홈페이지 발견률 (우리)** | **38.9%** `[29.5%, 49.2%]` | ≥ 70% | ❌ 미달 — 07-30 의 18.9% 에서 2배 (`homepage_discover` 효과) |
| M1-상한 실제 존재율 (사람 라벨) | 61.6% `[51.1%, 71.2%]` (n=86) | — | 기준 70% 는 **발주자 결정으로 유지** (D-006) — 격차는 소스 보강 몫 |
| **M2 공식 판별 정밀도** | **90.9%** `[72.2%, 97.5%]` (n=22) | ≥ 0.90 | ✅ **점추정 기준 충족** (08-04: 84.6%) |
| **M2 공식 판별 재현율** | **71.4%** `[52.9%, 84.7%]` (n=28) | ≥ 0.75 | 미달이나 근접 (08-04: 39.3% → +9건). 남은 FN 8건은 아래 |
| M3 연락처 페이지 적중률 | 14.3% `[2.6%, 51.3%]` (n=7) | ≥ 50% | n=7 — 판단 불가 |
| M3b 이메일 공개율 | 44.2% `[31.6%, 57.7%]` (n=52) | ≥ 30% (<20% → stop) | ✅ stop 아님 |
| M6 ORS 산출 가능률 | 25.6% `[17.7%, 35.4%]` | ≥ 90% | ❌ 미달 (08-04: 14.4%) — 공식 판정에 종속 |
| M7 ORS ↔ 체감 노출 ρ | 0.251 `[-0.180, 0.601]` (n=23) | CI 상한 <0.4 → stop | ✅ stop 아님 — 다만 데이터가 쌓일수록 낮아지는 추세다(0.495→0.338→0.251). Phase 4 확증 검증(n=240)에서 주시 |
...
**남은 M2 FN 8건의 성격** — 대부분 코드가 아니라 정책·소스의 몫이다:
