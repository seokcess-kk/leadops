...
### ❗ 실측된 것 (2026-08-04 · 실키 · shadow ORS · 라벨 90건 · 시드 42)
...
| M2 공식 판별 정밀도 | 84.6% `[57.8%, 95.7%]` (n=13) | ≥ 0.90 | 미달이나 CI 가 기준을 걸친다 — n 부족 |
| M2 공식 판별 재현율 | 39.3% `[23.6%, 57.6%]` (n=28) | ≥ 0.75 | ❌ 미달 |
| M3 연락처 페이지 적중률 | 16.7% `[3.0%, 56.4%]` (n=6) | ≥ 50% | n=6 — 판단 불가 |
| M3b 이메일 공개율 | 44.2% `[31.6%, 57.7%]` (n=52) | ≥ 30% (<20% → stop) | ✅ stop 아님 |
| M6 ORS 산출 가능률 | 14.4% `[8.6%, 23.2%]` | ≥ 90% | ❌ 미달 — 공식 판정(M2 재현율)에 종속 |
| **M7 ORS ↔ 체감 노출 ρ** | **0.495** `[-0.077, 0.822]` (n=13) | CI 상한 <0.4 → stop | ✅ **stop 아님** — 실 shadow ORS 로 측정 |
