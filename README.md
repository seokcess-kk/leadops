...
> **현재 상태: Phase 7 대부분 완료 (스케줄러·용량·개인정보 집행·복구 리허설·파티셔닝)**
> 설계서 `docs/00-plan.md` v3 기준. Phase 6 은 닫혔다(E2E 13건). Phase 7 에서 남은 것은
> **평일 06:00 3영업일 연속 성공**(달력이 필요하다)이다. 골드셋 90건은 측정됐다
> (2026-08-04 · 판정 inconclusive — 진행 허용, 아래 실측 표). 운영 절차는 [`docs/07-runbook.md`](docs/07-runbook.md).
...
| [`docs/03-decisions.md`](docs/03-decisions.md) | 발주자 의사결정 기록 (D-001 ~ D-006) |
...
| robots.txt **5xx·네트워크 실패**는 fail-closed — 4xx 는 RFC 9309 에 따라 허용 (D-005) | `packages/http/src/robots.ts` · `robotsGate.ts` |
...
| `NaverSearchAdapter` | ✅ **검증됨** (2026-07-31) | legacy 4채널 실검증·fixture 녹화 (`pnpm spike verify`). **API HUB 변형은 미검증**(R2-04) — 생성자가 거부한다. legacy 종료 2027-06-30 |
...
| 상호 + 시군구 텍스트 동시 일치 (webkr 폴백) | 채택 (약한 근거) — 지역검색이 근거를 못 찾았을 때만 |
| 그 외 | **거절** |

상호만 맞으면 동명이인이고 지역만 맞으면 옆 건물 다른 병원이다. 애그리게이터·SNS 링크는
**요청 전에** 자른다. 검색 순위는 그 업체 것이라는 근거가 아니므로 순위로 고르지 않는다.

지역검색이 이 근거를 못 찾으면 웹검색(webkr)으로 **1회 폴백**한다. 전화·주소가 없어
상호(접미 완화)와 시군구가 제목+설명에 함께 나타나야 채택하고,
`discovery_basis='name_region_text'` 로 남겨 사후에 근거별 정확도를 나눠 검증할 수 있다.
...
3. Supabase Auth 를 붙일 때 고친 곳이 `src/lib/server/token.ts` 하나였다 — 세션이 있으면
   항상 세션이 이기고, dev 토큰은 E2E·로컬 격리 경로로 남는다.
...
> ⚠️ **`LEADOPS_DEV_LOGIN` 은 E2E·로컬 전용 격리 경로다.** Supabase Auth 로그인(`/login`)이
> 붙었고(2026-08-03) 세션이 있으면 항상 세션이 이긴다. `NODE_ENV=production` 이거나 플래그가
> 없으면 `401 auth_unavailable` 로 **거부한다** — 조용히 폴백하지 않는다.
...
Outreach(발송 상태) 모듈 — 설계서 1.3 이 범위에서 제외했다 (확장 seam 만 있다).

Supabase Auth 는 붙었다 (2026-08-03) — taimen 로그인·로그아웃·미들웨어 세션 가드,
검수 API 의 ES256(Supabase JWT signing keys) 사용자 토큰 검증. `LEADOPS_DEV_LOGIN` 은
E2E 가 실 Supabase 에 의존하지 않기 위한 격리 경로로 유지한다.
...
Phase 2~5 의 완료 기준은 **골드셋 90건으로 측정됐다** (2026-08-04 · 아래 표). 골드셋이
120건이 아니라 90건인 것은 공정위 요청주소가 없어 프랜차이즈 30건을 못 뽑았기 때문이다.
점수 가중치 보정은 아직 하지 않았다.

### ❗ 실측된 것 (2026-08-07 측정 · 08-06 재크롤 · webkr 폴백 반영 · 실키 · shadow ORS · 라벨 90건 · 시드 42)

08-04 첫 측정(M2 재현율 39.3%) 이후 세 차례의 원인 분해·수정 사이클
(#13 접미 대조 · #14 리다이렉트 추적/재발견/Crawl-delay · #16 robots 4xx ·
#18 webkr 텍스트 근거 폴백)을 거친 현재 값이다.

| 지표 | 실측 | 기준 | 비고 |
|---|---:|---:|---|
| **M1 홈페이지 발견률 (우리)** | **70.0%** `[59.9%, 78.5%]` | ≥ 70% | ✅ **점추정 기준 도달** (07-30 18.9% → 08-05 38.9% → 70.0%, webkr 폴백 효과). 분석 업체도 661→1,092 |
| M1-상한 실제 존재율 (사람 라벨) | 61.6% `[51.1%, 71.2%]` (n=86) | — | M1 이 상한을 넘는 것은 M1 이 **후보 확보율**이기 때문 — 틀린 후보 몫은 판정이 거른다 (아래) |
| **M2 공식 판별 정밀도** | **91.7%** `[74.2%, 97.7%]` (n=24) | ≥ 0.90 | ✅ 충족 유지 — **webkr 채택분이 정밀도를 오염시키지 않았다** |
| M2 공식 판별 재현율 | 53.7% `[38.7%, 67.9%]` (n=41) | ≥ 0.75 | ⚠️ **구성 효과** — 절대 TP 는 20→22 로 늘었다. 분모가 28→41 로 커진 것은 webkr 가 판정권을 넓혔기 때문이고, 새 FN 13건 중 12건은 webkr 의 틀린 후보를 판정이 `not_official` 로 **올바르게 거른** 것이다 (아래 해설) |
| M3 연락처 페이지 적중률 | 11.1% `[2.0%, 43.5%]` (n=9) | ≥ 50% | n=9 — 판단 불가 |
| M3b 이메일 공개율 | 44.2% `[31.6%, 57.7%]` (n=52) | ≥ 30% (<20% → stop) | ✅ stop 아님 |
| M6 ORS 산출 가능률 | 27.8% `[19.6%, 37.8%]` | ≥ 90% | ❌ 미달 — 공식 판정에 종속 |
| M7 ORS ↔ 체감 노출 ρ | 0.293 `[-0.116, 0.617]` (n=25) | CI 상한 <0.4 → stop | ✅ stop 아님 — 추세 주시(0.495→0.338→0.251→0.293). Phase 4 확증 검증(n=240)에서 갈린다 |
| M8 경쟁사 선정 타당성 | 평균 3.99 · 3점 미만 `[15.0%, 32.2%]` | 평균 ≥3.5 · 3점 미만 ≤20% | 평균은 충족 · 비율 CI 가 기준을 걸친다 |
| M9 최종 적합률 | 88.9% `[80.7%, 93.9%]` | ≥ 60% | ✅ 충족 (업종별 83~97%) |
| M13 JS 전용 렌더링 비율 | 0.0% `[0.0%, 6.9%]` (n=52) | — | 참고 |
| M14 무료메일 대표주소 비율 | 82.6% `[62.9%, 93.0%]` (n=23) | — | 참고 |

**webkr 폴백이 설계대로 동작했다는 근거** (골드셋 표본 내 채택 30건의 최신 판정):
`not_official` 24 · `unavailable` 4 · `confirmed` 2. 발견이 넓게 던지고 다신호 판정이
좁게 거르는 분업 — 틀린 후보 24건이 리드로 새지 않고 전부 강등됐다. 재현율 %가
내려간 것은 이 강등분이 분모에 새로 들어왔기 때문이지 판정이 나빠진 것이 아니다.
(주의: 판정 통과 2건 중 1건은 라벨과 도메인이 다르다 — 도메인 일치 기준 TP 는 21이다.)

**판정: `inconclusive` — 중단 게이트(M3b·M7)를 넘지 않아 Phase 1 진행이 허용된다.**
R2-06 에 따라 'go' 판정은 없고, 이 결과로 `final_max`·ORS 배점을 정하지 않는다.

**남은 M2 FN 의 성격** (아래 표는 08-05 시점의 8건 분해 — webkr 도입으로 분모 구성이
바뀌어 건수는 달라졌지만, 기권·소스·설계 논의라는 성격 구분은 그대로 유효하다):

| 사유 | 건수 | 다음 행동 |
|---|---:|---|
| robots `Disallow` (사이트의 의사) | 2 | **고치지 않는다** — 정당한 기권 |
| 봇 차단 WAF (robots 403 **+ 페이지도 403**) | 1 | D-005 로 robots 게이트는 통과했으나(08-06 실측: `robots_allowed=true`) **같은 WAF 가 페이지 요청에도 403** 을 준다 (브라우저 UA 는 200). 사이트의 의사이므로 **UA 위장으로 우회하지 않는다** — 기권으로 재분류 |
| 죽은 hospUrl + 지역검색이 대체 도메인을 못 찾음 | 2 | 재발견은 돌았으나 근거(전화·상호+지역) 있는 후보가 없었다 — 소스 보강 몫 |
| 프랜차이즈 공유 도메인 (`shared_domain` 강등, 7개 업체 공유) | 1 | 규칙이 의도대로 동작한 것 — **현행 유지 결정 (D-007)**, 라벨링 기준을 정합시킨다 |
| 이미지 전용 껍데기 (리다이렉트 없음) | 1 | 신호 자체가 없다 |
| 서버가 깨진 302 를 줌 (`co.krrobots.txt`) | 1 | 사이트 결함 |

**표본 90건이 파이프라인을 어디까지 통과했는가** (2026-07-30 · `homepage_discover` 도입 전)
...
> ✅ **재측정됐다 (2026-08-04·05).** 네이버 자격증명 확보 후 M1 이 18.9% → **38.9%** 로
> 올랐다. 사람 라벨 기준 실제 홈페이지 존재율이 61.6% 이므로, **존재하는 홈페이지의 약
> 63% 를 찾는 셈**이다. 기준 70% 는 모집단 상한(61.6%)보다 높지만 **발주자 결정(D-006)으로
> 유지**한다 — 격차는 소스 보강으로 좁힌다. 공식 판별 재현율은 FN 분해 기반 수정
> 4건(#13·#14) 후 39.3% → **71.4%** 다.
...
| ~~라벨링 (90건)~~ | ✅ 완료 (2026-08-03 · 이중라벨 검증 포함) |
| ~~네이버 자격증명 + `FEATURE_ORS=shadow`~~ | ✅ 확보 · shadow 가동 · M7 실측 완료 (2026-08-04) |
