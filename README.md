...
**대응 방향** (설계서 9.1 의 M1 미달 대응 = "소스 보강"):
플레이스·검색으로 홈페이지를 찾는 발견 경로가 없으면 M1 이 19% 를 넘지 못한다.
다만 검색으로 찾으면 동명이인·팬페이지를 공식으로 오인하고 쿼터도 쓰므로, 그 위험을
설계에 반영해야 한다 (현재 채널 발견이 "홈페이지가 스스로 링크한 주소만 인정" 하는 것과
같은 종류의 판단이 필요하다).
