# NAVER API 서비스 이용약관 — 스냅샷 (서면 근거)

| | |
|---|---|
| 원본 | https://developers.naver.com/products/terms/ |
| 수집 일시 | 2026-07-31 (KST) |
| 약관 개정일 | 2026년 7월 31일 |
| 수집 목적 | D-002(네이버 Open API 사용) 재확인 — `source_registry['naver_search']` 서면 근거 |

## 검토 요약 (2026-07-31 · 발주자 재확인)

- **7.3②**: 스팸/광고 정보의 작성·전송 목적 이용 금지 → 해당 없음 — 이 시스템은 검색 결과를
  분석·검수에만 쓰고 광고 정보의 작성·전송에 쓰지 않는다 (발송 기능 자체가 MVP 범위 밖).
- **7.3③**: API 로 취득한 정보의 허용 범위 초과 "무단 복제·저장(캐시 포함)·가공·배포" 금지.
  예시로 "지역정보 수집 후 광고 영업 이용"·"별도 데이터베이스 관리" 명시.
  → 완화 요소: 본문 미저장(URL·제목·메타만) · 관련 문서 30일 보관 · 집계 중심 · 재배포 없음.
- **검색 API 특약 2.1**: 검색결과 독립 노출·왜곡 금지, URL 등 수정·변조 금지.
  → 내부 검수 화면에 원본 그대로 표시 (수정·변조 없음).
- **8조**: 결과 데이터의 권리는 회사/원저작자. 제한된 사용권만 부여.
- **부칙 (2026-07-31 시행)**: Search API 는 NAVER API HUB 로 이관.
  개발자센터 신규 접수는 2026-07-30 24:00 중단. **기존 이용자(발주자 — 7/30 이전 등록)는
  2027-06-30 24:00 까지** 현행 이용 가능. 이후는 API HUB 별도 절차·약관.
  쇼핑·책·학술정보 검색은 2026-07-31 24:00 종료 (미사용 — 무관).
- **12.2**: 3개월 미사용 시 해지 가능 (일 배치 사용 — 무관).

**결정**: D-002 "사용 가능" 유지 (발주자, 2026-07-31). 최종 해석 책임은 발주자.

---

## 전문

NAVER API 서비스 이용약관 - NAVER Developers
NAVER API 서비스 이용약관
2026년 7월 31일 개정
NAVER API 서비스 이용약관
가. 일반조건
...
⑦ 이용자는 애플리케이션 등을 이용고객에게 제공함에 있어 관련 법령을 준수하고 있음을 보증하며, 관련 법령에 따라 이용고객의 실명·나이 확인, 본인 확인, 법정대리인·친권자 동의 여부 확인 등 의무가 있는 경우 이를 이행하고 필요한 조치를 하여야 합니다. 또한, 이용자는 네이버 로그인 서비스 기능을 이용하여 제공 받는 일체의 네이버 회원 정보를 법령상 의무 준수를 위한 이용고객의 본인확인, 나이 확인 등 정보 확인에 활용하여서는 안됩니다.
...
<부칙>
제1조 (시행일)
본 부칙은 2026년 7월 31일부터 시행합니다.
제2조 (일부 API 서비스 제공 종료 및 이관에 따른 경과조치)
① 회사는 네이버 클라우드 플랫폼(NCP)의 NAVER API HUB로의 서비스 이관에 따라, 본 약관 시행일 이후인 2026년 7월 30일 24:00를 기점으로 네이버 개발자센터를 통한 'Search API', 'Search Trend API', 'Shopping Insight API'(이하 "이관 대상 API")의 신규 이용 신청 접수를 중단합니다.
② 제1항에도 불구하고, 2026년 7월 30일 24:00까지 이관 대상 API의 이용 신청을 완료한 기존 이용자에 한하여, 2027년 6월 30일 24:00까지 현행과 동일하게 네이버 개발자센터를 통해 해당 API 서비스를 제공합니다.
③ 제1항 및 제2항의 규정에도 불구하고, 네이버 개발자센터를 통한 'Search API' 중 '쇼핑', '책', '학술정보' 데이터 제공 서비스는 2026년 7월 31일 24:00부로 종료됩니다. 해당 서비스 종료 시점 기준 기존 이용자라 하더라도 2026년 8월 1일 00:00부터는 해당 API를 더 이상 이용할 수 없습니다.
